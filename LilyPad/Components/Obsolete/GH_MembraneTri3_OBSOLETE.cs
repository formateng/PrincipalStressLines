...
{
    public class GH_MembraneTri3_OBSOLETE : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the GH_MembraneQuad4 class.
        /// base(Name, Nickname, Description, Folder, SubFolder)
        /// </summary>
        public GH_MembraneTri3_OBSOLETE()
          : base("3 Node Triangular Membrane", "Tri3Mem",
              "Transforms Mesh and results into 3-node triangular membrane elements for principal stress line analysis using the theory of in-plane loaded plates and linear shape functions",
              "LilyPad", " Setup")
        {
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            for (int i = 0; i < iMesh.Faces.Count; i++)
            {
                ...
                Plane fitPlane;
                double maxDeviation;
                if (Plane.FitPlaneToPoints(new List<Point3d> { point1, point2, point3}, out fitPlane, out maxDeviation) != 0)
                {
                ...
                Tri3Element bilinearIsoPara1 = new Tri3Element(point1, point2, point3, iU[p1], iU[p2], iU[p3], iV, true);

                //output data
                bilinearIsoPara1.ChangeDirection(1);
                sigma1.Add(new Element(bilinearIsoPara1));

                Tri3Element bilinearIsoPara2 = new Tri3Element(point1, point2, point3, iU[p1], iU[p2], iU[p3], iV, true);
        ...
        {
            get { return new Guid("68ddf900-f69b-42b2-9255-298eea3ebfcb"); }
        }
